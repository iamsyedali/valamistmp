package com.arcusys.valamis.persistence.impl.social

import com.arcusys.valamis.model.Order
import com.arcusys.valamis.persistence.common.SlickProfile
import com.arcusys.valamis.persistence.impl.social.schema.CommentTableComponent
...
class CommentRepositoryImpl(
  ...
  def update(comment: Comment): Comment = db.withSession { implicit session =>
    comments.filter(_.id === comment.id).map(_.update).update(comment)
