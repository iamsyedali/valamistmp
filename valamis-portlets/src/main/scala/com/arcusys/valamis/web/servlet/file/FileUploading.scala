...
trait FileUploading extends ScalatraServlet with HasMultipartConfig {
  ...
  private def mergeFormParamsWithQueryString(req: HttpServletRequest, bodyParams: BodyParams) = {
    ...
    names.foreach {
      case n =>
        ...
        val reqParameters = Option(req.getParameterValues(n)).map(_.toList).getOrElse(Nil)
        mergedParams += n -> (reqParameters ++ formValues)
    }
    mergedParams
  }
