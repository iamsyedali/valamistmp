...
//  override def getAssetObject: LAssetEntry = assetEntry

  override def getSummary(portletRequest: PortletRequest, portletResponse: PortletResponse): String = HtmlUtilHelper.stripHtml(assetEntry.getSummary)

  override def include(httpServletRequest: HttpServletRequest, httpServletResponse: HttpServletResponse, s: String): Boolean = false

  override def getAssetObject: Nothing = ???
}
