...
class CourseNotificationDefinition
  extends UserNotificationDefinition(PortletName.MyCourses.key,
    0L,
    0,
    "notification.course") { //TODO: add localization

    this.addUserNotificationDeliveryType(new UserNotificationDeliveryType("website", 10002, true, true))

    override def getDescription(locale: Locale): String = LanguageHelper.get("notification.course")
}
