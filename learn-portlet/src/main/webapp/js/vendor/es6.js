...
(function (global) {
  ...
  define(
    ...
    function copyWithin(target, start/*, end*/) {
      ...
      var final_val;
      if (relativeEnd < 0)
        final_val = max(len + relativeEnd, 0);
      else
        final_val = min(relativeEnd, len);
      var count = min(final_val - from, len - to);
      var direction;
   ...
   'Float32Array', 'Float64Array'].forEach(function ($TypedArrayName$) {
     ...
     define(
       ...
       function fill(value/*, start, end*/) {
         ...
         var final_val;
         if (relativeEnd < 0) final_val = max((len + relativeEnd), 0);
         else final_val = min(relativeEnd, len);
         while (k < final_val) {
           var pk = String(k);
           o[pk] = value;
           k += 1;
     ...
     define(
       ...
       function slice(start, end) {
         ...
         var final_val = (relativeEnd < 0) ? max(len + relativeEnd, 0) : min(relativeEnd, len);
         var count = final_val - k;
         var c = o.constructor;
         if (IsConstructor(c)) {
           var a = new c(count);
         } else {
