package com.arcusys.learn.liferay.update.version240.storyTree
...
trait StoryTreeTableComponent {
  ...
  type Story = (Option[Long], Long, String, String, Option[String], Boolean)
  class StoryTreeTable(tag : Tag) extends Table[Story](tag, tblName("STORY_TREE")) {
    ...
    def * = (id.?, courseId, title, description, logo, published)
  }

  type StoryNode = (Option[Long], Option[Long], Long, String, String)
  class StoryTreeNodeTable(tag : Tag) extends Table[StoryNode](tag, tblName("STORY_TREE_NODE")) {
    ...
    def * = (id.?, parentId, treeId, title, description)
