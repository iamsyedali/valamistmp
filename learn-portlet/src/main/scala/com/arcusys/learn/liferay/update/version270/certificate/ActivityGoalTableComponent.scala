...
trait ActivityGoalTableComponent extends CertificateTableComponent { self: SlickProfile =>
  ...
  type ActivityGoal = (Long, String, Int, Int, PeriodTypes.Value, Int, Option[Long])
  class ActivityGoalTable(tag: Tag) extends Table[ActivityGoal](tag, tblName("CERT_ACTIVITY_GOAL")) {
    ...
    def * = (certificateId, activityName, count, periodValue, periodType, arrangementIndex, id.?)

    def PK = primaryKey(pkName("CERT_ACTIVITY_GOAL"), id)
    def idx = index("CERT_ACTIVITY_GOAL_INDEX", (certificateId, activityName), unique = true)
